...
from QNetworks import QNet_baseline, QNet_3hidden, QNet_dropout, QNet_dueling
...
class Double_DQN_Agent():
    ...
    def __init__(self, state_size, action_size, seed, qnet="baseline", filename=None):
        ...
        if qnet=="3hidden":
            ...
            self.qnetwork_local = QNet_dropout(state_size, action_size, seed).to(device)
            self.qnetwork_target = QNet_dropout(state_size, action_size, seed).to(device)
            self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=LR)
        elif qnet=="dueling":
            self.qnetwork_local = QNet_dueling(state_size, action_size, seed).to(device)
            self.qnetwork_target = QNet_dueling(state_size, action_size, seed).to(device)
            self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=LR)
        else:
